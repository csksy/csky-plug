...
object AniShowsExtractors {
    ...
    suspend fun invokeAllSources(
        ...
        val executionList = ProviderRegistry.keys.mapNotNull { key ->
        ...
        runLimitedAsync(concurrency = 15, *executionList.toTypedArray())
    }
    ...
    private fun getDynamicStremioMap(
        ...
        subtitleCallback: (SubtitleFile) -> Unit,
        callback: (ExtractorLink) -> Unit
    ): Map<String, suspend () -> Unit> = emptyMap()
    ...
    suspend fun invokeWYZIESubs(
        ...
        val url = if(season != null) "$WYZIESubsAPI/search?id=$id&season=$season&episode=$episode&source=all&key=${"" ?: return}" else "$WYZIESubsAPI/search?id=$id&source=all&key=${"" ?: return}"
    ...
    suspend fun invokeBollywood(
        ...
        val headers = mapOf(
            ...
            "User-Agent" to USER_AGENT,
            "Authorization" to "Bearer ${"" ?: return}"
        )
    ...
    suspend fun invokeStreamioStreamsGlobal(
        ...
        parseJson<StreamifyResponse>(json).streams.forEach { s ->
            ...
            if(streamUrl.contains("video-downloads.googleusercontent") && true == false) return@forEach
