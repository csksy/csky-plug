...
/**
 * Cloudflare bypass for prox.anikage.cc — the resolver the site's own player uses for the
 * token-only servers (kiwi / uwu / megg / dib / wave). The prox sits behind a Cloudflare
 * challenge that plain HTTP clients cannot pass; a WebView solves it once, the resulting
 * cf_clearance cookie (+ matching UA) is stored and attached to every emitted prox link so
 * ExoPlayer passes the edge. Pattern adapted from the proven AniDb/Anidap bypass.
 */
...
/**
 * GET with auto Cloudflare-bypass for prox.anikage.cc. Returns the response whether or
 * not the bypass succeeded (callers treat "bad url"/"invalid payload" app responses as
...
/** Cookie/UA headers to attach to emitted prox playback links (empty when no bypass yet). */
