...
class EnmaProvider : MainAPI() {
    ...
    private suspend fun resolveMegaPlay(
        ...
        try {
            ...
            val m3u8 = when {
                !enc.isNullOrBlank() -> EnmaCipher.decryptFile(enc)
                    ?: return false
                else -> {
                    val sourcesEl = root.get("sources")
                    when {
                        sourcesEl?.isJsonObject == true -> sourcesEl.asJsonObject.get("file")?.asString
                        sourcesEl?.isJsonArray == true && sourcesEl.asJsonArray.size() > 0 ->
    ...
    private suspend fun resolve4Animo(
        ...
        try {
            ...
            val html = app.get(iframeUrl, headers = pageHeaders("$mainUrl/")).text
            ...
            val sourcesArr = root.getAsJsonArray("sources") ?: return false
            if (sourcesArr.size() == 0) return false
            val file = sourcesArr[0].asJsonObject.get("file")?.asString ?: return false
