...
class KMMoviesSettingsFragment(private val plugin: Plugin) : BottomSheetDialogFragment() {

    private val TAG = "NKM_Settings"
    ...
    override fun onCreateView(
        ...
        val bypassBtn = Button(ctx).apply {
            text = if (NewKMMoviesPlugin.cfCookies.isNotBlank()) {
        ...
        bypassBtn.setOnClickListener {
            val host = NewKMMoviesPlugin.cfCookieHost.ifBlank { "https://kmmovies.online" }
            ...
            CoroutineScope(Dispatchers.Main).launch {
                try {
                    val success = showKMCFBypassDialogAndWait(bypassUrl)
                    bypassBtn.text = if (success && NewKMMoviesPlugin.cfCookies.isNotBlank()) {
                        "CF Cookies Saved - Refresh"
                    } else {
                        "Bypass Cloudflare"
                    }
        ...
        clearBtn.setOnClickListener {
            AlertDialog.Builder(ctx)
                ...
                .setPositiveButton("Clear") { _, _ ->
                    val host = NewKMMoviesPlugin.cfCookieHost
                    if (host.isNotBlank()) {
                        try {
                            val cm = android.webkit.CookieManager.getInstance()
                            listOf("cf_clearance", "cf_chl_rc_ni", "cf_chl_prog").forEach { name ->
