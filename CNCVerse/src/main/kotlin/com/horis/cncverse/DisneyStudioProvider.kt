...
// Parameterised clone of HotStarMirrorProvider that the plugin instantiates four
// times — once per Disney-owned studio (Disney, Marvel, Star Wars, Pixar). The
// only studio-specific discriminator is the `studio` cookie appended below; the
// URL prefix, OTT cookie and image paths all reuse the HotStar surface on the
// shared NetMirror backend. Stream resolution routes through the rotating NewTv
// API with the same OTT code ("hs") as HotStar — the NewTv backend lumps
// Disney+HotStar content together, while the net52.cc catalogue distinguishes
// them via the `studio` cookie.
class DisneyStudioProvider(
    ...
    // `dp` selects the Disney+ catalogue on net52.cc; the studio cookie (when
    // non-empty) further narrows it to a single Disney-owned studio. `hd=on`
    // asks the backend for the HD variant.
    private fun netmirrorCookies(): Map<String, String> = buildMap {
        put("t_hash_t", cookie_value)
        put("ott", "dp")
        put("hd", "on")
    ...
    // HotStar-style image scheme is reused for the Disney catalogue too — same
    // `hs/v/` poster path, `hs/h/` backdrop path, and the load-vs-getEpisodes
    // episode-thumbnail inconsistency (`hsepimg/150/{id}` vs `hsepimg/{id}`)
    // is preserved because the backend serves different crops at each path.
    private fun posterUrl(id: String): String = "https://imgcdn.kim/hs/v/$id.jpg"
    private fun backdropUrl(id: String): String = "https://imgcdn.kim/hs/h/$id.jpg"
    private fun episodePosterUrl(id: String): String = "https://imgcdn.kim/hsepimg/150/$id.jpg"
    private fun episodesListPosterUrl(id: String): String = "https://imgcdn.kim/hsepimg/$id.jpg"
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val postUrl = "$mainUrl/mobile/hs/post.php?id=$id&t=${APIHolder.unixTime}"
        val resp = app.get(
            url = postUrl,
            headers = headers,
        ...
        return newTvSeriesLoadResponse(
            ...
            posterUrl = posterUrl(id)
            backgroundPosterUrl = backdropUrl(id)
            posterHeaders = posterHeaders()
            plot = data.desc
    ...
    private suspend fun getEpisodes(
        ...
        var pg = page
        while (true) {
            val url = "$mainUrl/mobile/hs/episodes.php?s=$sid&series=$eid&t=${APIHolder.unixTime}&page=$pg"
            val resp = app.get(
                url = url,
                headers = headers,
                referer = "$mainUrl/home",
    ...
    override suspend fun loadLinks(
        ...
        return try {
            ...
            // The NewTv backend serves Disney+HotStar content under the shared
            // "hs" OTT code; the per-studio discrimination happens only on the
            // net52.cc catalogue via the studio cookie, not at the player API.
            // First attempt uses the cached token; if the server rejects it we
            // force an OTP refresh and retry exactly once.
            var userToken = getNewTvUserToken(apiBase, "hs", forceRefresh = false)
            ...
            Log.e("DisneyStudioProvider", "loadLinks failed: ${e.message}")
            false
        }
    }
    ...
    private suspend fun fetchPlayerResponse(
        ...
        val headers = buildNewTvHeaders("hs", mapOf("Usertoken" to userToken))
