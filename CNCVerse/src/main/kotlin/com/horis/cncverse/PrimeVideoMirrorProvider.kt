...
// Prime Video catalog on the shared NetMirror backend. The host (net52.cc) is
// the same as the Netflix sibling; the `ott=pv` cookie and the `/mobile/pv/`
// URL prefix are the only things that route requests to the Prime Video
// catalog. Discovery uses the cheap verify.php bypass (UtilsKt.bypass); stream
// resolution goes through the rotating NewTv API (UtilsKt.resolveApiUrl +
// getNewTvUserToken).
class PrimeVideoMirrorProvider : MainAPI() {
    ...
    // for the HD variant; `ott=pv` selects the Prime Video catalog.
    ...
    // Prime Video's image scheme is consistent: `pv/v/{id}` for vertical posters,
    // `pv/h/{id}` for horizontal backdrops, and `pvepimg/{id}` for episode thumbs.
    private fun posterUrl(id: String): String = "https://imgcdn.kim/pv/v/$id.jpg"
    private fun backdropUrl(id: String): String = "https://imgcdn.kim/pv/h/$id.jpg"
    private fun episodePosterUrl(id: String): String = "https://imgcdn.kim/pvepimg/$id.jpg"
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val postUrl = "$mainUrl/mobile/pv/post.php?id=$id&t=${APIHolder.unixTime}"
        val resp = app.get(
            url = postUrl,
            headers = headers,
            referer = "$mainUrl/home",
    ...
    private suspend fun getEpisodes(
        ...
        var pg = page
        while (true) {
            val url = "$mainUrl/mobile/pv/episodes.php?s=$sid&series=$eid&t=${APIHolder.unixTime}&page=$pg"
            val resp = app.get(
                url = url,
                headers = headers,
                referer = "$mainUrl/home",
    ...
    override suspend fun loadLinks(
        ...
        return try {
            ...
            var userToken = getNewTvUserToken(apiBase, "pv", forceRefresh = false)
            ...
            Log.e("PrimeVideoMirrorProvider", "loadLinks failed: ${e.message}")
            false
        }
    }
    ...
    private suspend fun fetchPlayerResponse(
        ...
        val headers = buildNewTvHeaders("pv", mapOf("Usertoken" to userToken))
