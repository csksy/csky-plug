...
// HotStar catalog on the shared NetMirror backend. Same host (net52.cc) as the
// Netflix/PrimeVideo siblings; the `ott=hs` cookie and the `/mobile/hs/` URL
// prefix are the only things that route requests to the HotStar catalog.
// Discovery uses the cheap verify.php bypass (UtilsKt.bypass); stream resolution
// goes through the rotating NewTv API (UtilsKt.resolveApiUrl + getNewTvUserToken).
class HotStarMirrorProvider : MainAPI() {
    ...
    // Cookie bundle sent to every NetMirror endpoint. `hd=on` asks the backend
    // for the HD variant; `ott=hs` selects the HotStar catalog.
    ...
    // HotStar's image scheme: `hs/v/{id}` for vertical posters, `hs/h/{id}` for
    // horizontal backdrops. Episode thumbs use `hsepimg` — note the inconsistency
    // between the load path (`/150/{id}`) and the getEpisodes path (no `/150/`),
    // preserved here because the backend serves different crops at each path.
    ...
    override suspend fun loadLinks(
        ...
        return try {
            ...
            Log.e("HotStarMirrorProvider", "loadLinks failed: ${e.message}")
            false
        }
    }
