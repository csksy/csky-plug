...
class ReAnimeProvider : MainAPI() {
    ...
    companion object {
        // resolved master url -> playback context for the video interceptor
        private val streamContexts = object : LinkedHashMap<String, FlixStreamContext>(32, 0.75f, false) {
            override fun removeEldestEntry(
                eldest: MutableMap.MutableEntry<String, FlixStreamContext>
            ): Boolean = size > 40
        }
    }
                ...
                async {
                    ...
                    if (seenUrls.add(res.m3u8)) {
                        ...
                        val height = Regex("""RESOLUTION=\d+x(\d+)""")
                            ...
                            .maxOrNull()
                        val context = FlixStreamContext(res.pkKey, lang == "dub")
                        synchronized(streamContexts) {
                            streamContexts[res.m3u8] = context
                            streamContexts[res.m3u8.substringBefore("?")] = context
                        }
                        callback.invoke(
                            newExtractorLink(label, label, res.m3u8, type = ExtractorLinkType.M3U8) {
    ...
    override fun getVideoInterceptor(extractorLink: ExtractorLink): Interceptor? {
        val context = synchronized(streamContexts) {
            streamContexts[extractorLink.url]
                ?: streamContexts[extractorLink.url.substringBefore("?")]
        } ?: return null
        return FlixStreamInterceptor(context)
    }
}
