...
class RaghavNineAnime : MainAPI() {
    ...
    suspend fun findEpisodeLink(
        ...
        for (query in queries.distinct()) {
            val results = try {
                ...
                Log.e("RaghavAnime", "[9anime] search '$query' failed: ${e.message}")
                continue
            }
        ...
        if (candidates.isEmpty()) {
            Log.d("RaghavAnime", "[9anime] no title match for '${searchTitles.firstOrNull()}' ep $episode ${if (isDub) "dub" else "sub"}")
            return null
        }
        ...
        for (cand in usable.sortedByDescending { it.score }.distinctBy { it.url }) {
            try {
                ...
                Log.d("RaghavAnime", "[9anime] matched '${cand.name}' (dub=${cand.dub}) for ep $episode")
                return ep.data
            } catch (e: Throwable) {
                Log.e("RaghavAnime", "[9anime] load failed for '${cand.name}': ${e.message}")
            }
        }
        Log.d("RaghavAnime", "[9anime] ep $episode not found on ${usable.size} candidates")
        return null
    }
    ...
    override suspend fun loadLinks(
        ...
        val res = try {
            ...
            Log.e("RaghavAnime", "[9anime] watch page failed for ${data.take(90)}: ${e.message}")
        ...
        Log.d("RaghavAnime", "[9anime] ${embedUrls.size} mirrors for ${data.take(80)}")
        ...
        val results = embedUrls.map { embedUrl ->
            async {
                try {
                    ...
                    Log.e("RaghavAnime", "[9anime] mirror ${embedUrl.take(90)} failed: ${e.message}")
                    false
                }
            }
        }
    ...
    private suspend fun resolveMirror(
        ...
        return when {
            ...
            embedUrl.contains("megaplay.buzz") || embedUrl.contains("vidtube.site") || embedUrl.contains("vidwish.live") -> {
                ...
                if (stream == null) {
                    Log.d("RaghavAnime", "[9anime] megaplay gave no stream for ${embedUrl.take(80)}")
                    false
                } else {
            ...
            embedUrl.contains("vidmoly.biz") -> {
                ...
                if (m3u8 == null) {
                    Log.d("RaghavAnime", "[9anime] vidmoly gave no m3u8")
                    return false
                }
            ...
            embedUrl.contains("bysesayeveum.com") -> {
                try {
                    ...
                    Log.e("RaghavAnime", "[9anime] moon webview failed: ${e.message}")
                    false
                }
            }
            else -> {
