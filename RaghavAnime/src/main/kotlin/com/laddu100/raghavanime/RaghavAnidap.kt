...
class RaghavAnidap : MainAPI() {
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        Log.d("RaghavAnime", "[Anidap] search query=\"$query\"")
        ...
        for (attempt in 1..3) {
            try {
                ...
                if (body.contains("error", ignoreCase = true) && !body.contains("results")) {
                    ...
                    Log.e("RaghavAnime", "[Anidap] search attempt $attempt: $lastError")
                    kotlinx.coroutines.delay(2000L * attempt)
                    continue
                }
                val parsed = parseJson<SearchResponseData>(body)
                val results = parsed.results ?: emptyList()
                Log.d("RaghavAnime", "[Anidap] search attempt $attempt ok: ${results.size} results")
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        return try {
            ...
            Log.d("RaghavAnime", "[Anidap] detail animeId=$animeId slug=$slug totalEps=$totalEps")
            ...
            Log.d("RaghavAnime", "[Anidap] servers ep=1 code=${serversRes.code} length=${serversRes.text.length}")
            val servers = if (serversRes.code == 200 && !serversRes.text.contains("bot_detected") && !serversRes.text.contains("\"error\"")) {
                try { parseJson<ServersResponse>(serversRes.text) } catch (e: Exception) {
                    Log.e("RaghavAnime", "[Anidap] servers parse failed: ${e.message}")
                    ServersResponse()
                }
            ...
            Log.d("RaghavAnime", "[Anidap] servers: ${subProviders.size} sub providers, ${dubProviders.size} dub providers")
            ...
            Log.d("RaghavAnime", "[Anidap] load animeId=$animeId done: subEps=${subEpisodes.size} dubEps=${dubEpisodes.size}")
            ...
            Log.e("RaghavAnime", "[Anidap] load animeId=$animeId failed: ${e.message}")
            null
        }
    }
    ...
    suspend fun loadLinksByAnilistId(
        ...
        Log.d("RaghavAnime", "[Anidap] loadLinksByAnilistId anilistId=$anilistId ep=$episode isDub=$isDub")
        try {
            val detailRes = app.get("$mainUrl/api/anime/$anilistId", headers = baseHeaders, timeout = 15_000L)
            val detailRoot = parseJson<com.fasterxml.jackson.databind.JsonNode>(detailRes.text)
            val dataNode = detailRoot.path("data")
            if (dataNode.isMissingNode) {
                Log.d("RaghavAnime", "[Anidap] anilistId=$anilistId no data node in detail response")
                return false
            }
            val detail = parseJson<AnimeDetail>(dataNode.toString())
            val slug = detail.slug ?: detail.id
            if (slug == null) {
                Log.d("RaghavAnime", "[Anidap] anilistId=$anilistId detail has no slug/id")
                return false
            }
            ...
            Log.d("RaghavAnime", "[Anidap] anilistId=$anilistId ep=$episode ${providers.size} ${if (isDub) "dub" else "sub"} providers")
            ...
            for (provider in providers) {
                try {
                    ...
                    if (sourcesRes.code != 200 || sourcesRes.text.contains("bot_detected") || sourcesRes.text.contains("\"error\"")) {
                        Log.d("RaghavAnime", "[Anidap] provider ${provider.id} sources rejected: code=${sourcesRes.code}")
                        continue
                    }
                    val sourcesData = try { parseJson<SourcesResponse>(sourcesRes.text) } catch (e: Exception) {
                        Log.e("RaghavAnime", "[Anidap] provider ${provider.id} sources parse failed: ${e.message}")
                        continue
                    }
                    ...
                    for (source in sources) {
                        ...
                        Log.d("RaghavAnime", "[Anidap] provider ${provider.id} ep $episode: link \"$label\"")
                        found = true
                    }
                } catch (e: Exception) {
            ...
            Log.d("RaghavAnime", "[Anidap] loadLinksByAnilistId anilistId=$anilistId ep=$episode done found=$found")
            return found
        } catch (e: Exception) {
    ...
    override suspend fun loadLinks(
        ...
        Log.d("RaghavAnime", "[Anidap] loadLinks data=${data.take(80)}")
        val cleanData = data.removePrefix("$mainUrl/").removePrefix("$mainUrl|").trim()
        val parts = cleanData.split("|")
        if (parts.size < 5) {
            Log.d("RaghavAnime", "[Anidap] loadLinks: malformed data (${parts.size} parts)")
            return false
        }
        val slug = parts[0]
        val epNum = parts[1]
        ...
        for (providerId in providerIds) {
            ...
            try {
                ...
                if (sourcesRes.code != 200 || sourcesRes.text.contains("bot_detected") || sourcesRes.text.contains("\"error\"")) {
                    Log.d("RaghavAnime", "[Anidap] provider $providerId sources rejected: code=${sourcesRes.code}")
                    continue
                }
                ...
                for (source in sources) {
                    ...
                    when {
                        isM3u8 -> {
                            ...
                            Log.d("RaghavAnime", "[Anidap] provider $providerId ep $epNum: m3u8 link \"$label\"")
                            found = true
                        }
                        isMp4 -> {
                            callback.invoke(
                            ...
                            Log.d("RaghavAnime", "[Anidap] provider $providerId ep $epNum: mp4 link \"$label\"")
                            found = true
                        }
                        else -> {
                            val referer = apiHeaders["Referer"] ?: apiHeaders["referer"] ?: "$mainUrl/"
                            Log.d("RaghavAnime", "[Anidap] provider $providerId ep $epNum: extractor attempt ${sourceUrl.take(60)}")
                            val loaded = try {
                                loadExtractor(sourceUrl, referer, subtitleCallback, callback)
                            } catch (_: Exception) {
                                Log.e("RaghavAnime", "[Anidap] extractor failed for ${sourceUrl.take(60)}")
                                false
                            }
                            if (loaded) {
                                found = true
                            } else {
                                Log.d("RaghavAnime", "[Anidap] extractor returned false, emitting direct link \"$label\"")
                                callback.invoke(
                                    newExtractorLink(label, label, sourceUrl, type = ExtractorLinkType.VIDEO) {
                                        this.headers = apiHeaders
                                        this.quality = qualityInt
                ...
                Log.e("RaghavAnime", "[Anidap] provider $providerId failed: ${e.message}")
            }
        }

        Log.d("RaghavAnime", "[Anidap] loadLinks slug=$slug ep=$epNum done found=$found")
        return found
    }
}
