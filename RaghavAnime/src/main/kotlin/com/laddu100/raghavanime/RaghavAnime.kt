...
class RaghavAnime : MainAPI() {
    ...
    private suspend fun findEpisodeData(
        ...
        // Two-pass matching: try exact title matches (score 2) first, then
        // partial matches (score 1). This prevents wrong-season matching like
        // "Bleach" → "Bleach TYBW" when the user wants the original series.
        for (pass in 0..1) {
            val minScore = if (pass == 0) 2 else 1
            for (cand in allCandidates) {
                if (cand.titleScore < minScore) continue
                try {
