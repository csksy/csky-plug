...
internal object AniSnatchApi {
    ...
    private fun looksLikeChallenge(text: String): Boolean =
        text.contains("Just a moment") || text.contains("cf_chl_") || text.contains("challenge-platform")

    private val killerMap = ConcurrentHashMap<String, CloudflareKiller>()

    private fun killerFor(url: String): CloudflareKiller =
        killerMap.getOrPut(java.net.URI(url).host ?: ORIGIN) { CloudflareKiller() }

    private fun getWithCookies(url: String, referer: String?): String {
        val req = Request.Builder().url(url)
            .apply { baseHeaders(referer).forEach { (k, v) -> header(k, v) } }
            .build()
    ...
    /*
     * Falls back to the WebView solver when Cloudflare challenges the page
     * (rate triggered, mostly on datacenter ips).
     */
    private suspend fun getHtml(url: String, referer: String? = null): String {
        var body = try {
            getWithCookies(url, referer)
        } catch (e: Exception) {
            ""
        }
        if (body.isNotEmpty() && !looksLikeChallenge(body)) return body

        return withContext(Dispatchers.Main) {
            try {
                val killer = killerFor(url)
                val resp = app.get(url, headers = baseHeaders(referer), interceptor = killer)
                if (resp.code in 200..399 && !looksLikeChallenge(resp.text)) {
                    resp.text
                } else {
                    ""
                }
    ...
    private suspend fun api(endpoint: String, payload: JSONObject): JSONObject {
        ...
        while (true) {
            ...
            val req = Request.Builder().url(built.url)
                .apply { baseHeaders("$ORIGIN/home").forEach { (k, v) -> header(k, v) } }
