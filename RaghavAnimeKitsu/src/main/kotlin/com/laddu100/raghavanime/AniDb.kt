...
class AniDb : MainAPI() {
    ...
    private fun searchResponseBuilder(res: Document): List<AnimeSearchResponse> {
        ...
        Log.d("RaghavAnimeKitsu", "[AniDb] parsed ${results.size} anime-card results")
        return results
    }

    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        mainUrl = FirebaseDomainHelper.getDomain("anidb") ?: mainUrl
        Log.d("RaghavAnimeKitsu", "[AniDb] getMainPage '${request.name}' page $page on $mainUrl")
    ...
    override suspend fun search(query: String, page: Int): SearchResponseList {
        mainUrl = FirebaseDomainHelper.getDomain("anidb") ?: mainUrl
        Log.d("RaghavAnimeKitsu", "[AniDb] search '$query' on $mainUrl")
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        if (epResponse == null) Log.d("RaghavAnimeKitsu", "[AniDb] episodes api parse failed for $episodesUrl")
        val episodesList = epResponse?.episodes ?: emptyList()
        Log.d("RaghavAnimeKitsu", "[AniDb] episodes api returned ${episodesList.size} episodes")

        val firstEpId = episodesList.firstOrNull()?.id
        var hasSub = true
        var hasDub = false

        if (firstEpId == null) Log.d("RaghavAnimeKitsu", "[AniDb] episode list empty, defaulting sub=true dub=false")
        if (firstEpId != null) {
            ...
            Log.d("RaghavAnimeKitsu", "[AniDb] first episode languages: ${langs.size} langs -> hasSub=$hasSub hasDub=$hasDub")
        }
        ...
        Log.d("RaghavAnimeKitsu", "[AniDb] metadata malId=$malId anilistId=$anilistId parsed=${animeMetaData != null} (${animeMetaData?.episodes?.size ?: 0} meta episodes)")
        ...
        Log.d("RaghavAnimeKitsu", "[AniDb] parsed ${subEpisodes.size} sub / ${dubEpisodes.size} dub episodes for '$title' (isMovie=$isMovie)")
    ...
    override suspend fun loadLinks(
        ...
        Log.d("RaghavAnimeKitsu", "[AniDb] loadLinks data '${data.take(80)}'")
        ...
        Log.d("RaghavAnimeKitsu", "[AniDb] resolving episode $episodeId (audio '$audio', slug '$slug')")
        ...
        Log.d("RaghavAnimeKitsu", "[AniDb] langsToExtract=${langsToExtract.size} for audio '$audio'")
        ...
        langsToExtract.amap { language ->
            ...
            Log.d("RaghavAnimeKitsu", "[AniDb] lang '${language.name}' embed fetched: ${embedUrl.take(80)} (html len ${embedDoc.length})")
            ...
            if (hlsUrl != null) {
                Log.d("RaghavAnimeKitsu", "[AniDb] lang '${language.name}' direct hls: ${hlsUrl.take(80)}")
                ...
                Log.d("RaghavAnimeKitsu", "[AniDb] lang '${language.name}' no hls in embed html (len ${embedDoc.length}), trying loadExtractor")
                loadExtractor(embedUrl, "$mainUrl/", subtitleCallback, callback)
            }
        }

        Log.d("RaghavAnimeKitsu", "[AniDb] loadLinks done for episode $episodeId")
        return true
    }
