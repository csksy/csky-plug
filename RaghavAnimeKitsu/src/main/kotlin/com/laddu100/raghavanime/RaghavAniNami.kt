...
class RaghavAniNami : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val anilistId = Regex("""/anime/(\d+)""").find(url)?.groupValues?.get(1)?.toIntOrNull()
            ?: return null
        Log.d("RaghavAnimeKitsu", "[AniNami] load: anilistId=$anilistId url=${url.take(120)}")
        ...
        Log.d("RaghavAnimeKitsu", "[AniNami] load: episodes response len=${epsText.length}")
        val providers = try {
        ...
        Log.d("RaghavAnimeKitsu", "[AniNami] load: ${providers.size} providers: ${providers.keys.joinToString(",")}")
        ...
        Log.d("RaghavAnimeKitsu", "[AniNami] load: built ${subEpisodes.size} sub / ${dubEpisodes.size} dub episodes")
    ...
    override suspend fun loadLinks(
        ...
        Log.d("RaghavAnimeKitsu", "[AniNami] loadLinks: audio=$requestedAudio epIds=${epIds.size}")
        if (epIds.isEmpty()) {
        ...
        for (epId in epIds) {
            ...
            Log.d("RaghavAnimeKitsu", "[AniNami] provider=$provider: GET ${watchUrl.take(120)}")
            val streamsText = try {
            ...
            Log.d("RaghavAnimeKitsu", "[AniNami] provider=$provider: ${streams.size} streams")
        ...
        Log.d("RaghavAnimeKitsu", "[AniNami] loadLinks done: found=$found (seenUrls=${seenUrls.size})")
        return found
    }
