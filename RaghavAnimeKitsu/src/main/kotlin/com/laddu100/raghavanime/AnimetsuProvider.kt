...
class AnimetsuProvider : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        Log.d("RaghavAnimeKitsu", "[Animetsu] getMainPage '${request.name}' page $page on $mainUrl")
        val items = try {
            ...
            Log.e("RaghavAnimeKitsu", "[Animetsu] getMainPage '${request.name}' failed: ${e.message}")
            emptyList()
        }
        Log.d("RaghavAnimeKitsu", "[Animetsu] getMainPage '${request.name}' parsed ${items.size} items")
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        Log.d("RaghavAnimeKitsu", "[Animetsu] search '$query' on $mainUrl")
        return try {
            ...
            Log.d("RaghavAnimeKitsu", "[Animetsu] search '$query' -> ${results.size} results")
            results
        } catch (e: Exception) {
            Log.e("RaghavAnimeKitsu", "[Animetsu] search '$query' failed: ${e.message}")
            emptyList()
        }
    }
    ...
    override suspend fun load(url: String): LoadResponse? {
        mainUrl = FirebaseDomainHelper.getDomain("animetsu") ?: mainUrl
        Log.d("RaghavAnimeKitsu", "[Animetsu] load $url on $mainUrl")
        ...
        val info = try {
            ...
            Log.e("RaghavAnimeKitsu", "[Animetsu] info fetch failed for '$animeId': ${e.message}")
            return null
        }
        ...
        val eps = try {
            ...
            Log.e("RaghavAnimeKitsu", "[Animetsu] episode list fetch failed for '$animeId': ${e.message}")
            emptyList()
        }
        Log.d("RaghavAnimeKitsu", "[Animetsu] load '$animeId': ${eps.size} episodes")
            ...
            try {
                ...
                for (server in servers) {
                    try {
                        ...
                        Log.e("RaghavAnimeKitsu", "[Animetsu] dub probe failed on server '${server.id}': ${e.message}")
                    }
                }
            } catch (e: Exception) {
                Log.e("RaghavAnimeKitsu", "[Animetsu] dub detection failed for '$animeId': ${e.message}")
            }
        }
        Log.d("RaghavAnimeKitsu", "[Animetsu] load '$animeId': dub available=$hasDub")
    ...
    override suspend fun loadLinks(
        ...
        Log.d("RaghavAnimeKitsu", "[Animetsu] loadLinks anime=$animeId ep=$epNum type=$sourceType")

        val servers = try {
            parseJson<List<ServerItem>>(apiGet("$apiBase/servers/$animeId/$epNum"))
        } catch (e: Exception) {
            Log.e("RaghavAnimeKitsu", "[Animetsu] server list fetch failed for ep $epNum: ${e.message}")
            return false
        }
        Log.d("RaghavAnimeKitsu", "[Animetsu] loadLinks ep $epNum: ${servers.size} servers")
        ...
        for (server in servers) {
            try {
                ...
                Log.d("RaghavAnimeKitsu", "[Animetsu] server '${server.id}' returned ${sources.size} sources")
                ...
                Log.e("RaghavAnimeKitsu", "[Animetsu] server '${server.id}' failed: ${e.message}")
            }
        }

        Log.d("RaghavAnimeKitsu", "[Animetsu] loadLinks ep $epNum done, found=$found")
        return found
    }
