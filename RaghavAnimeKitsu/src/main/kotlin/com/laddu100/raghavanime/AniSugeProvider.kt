...
class AniSugeProvider : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        Log.d("RaghavAnimeKitsu", "[AniSuge] getMainPage '${request.name}' page $page on $mainUrl")
        ...
        if (section == null) Log.d("RaghavAnimeKitsu", "[AniSuge] getMainPage no section matched '${request.data}'")
        ...
        Log.d("RaghavAnimeKitsu", "[AniSuge] getMainPage '${request.name}' parsed ${home.size} items")
        return newHomePageResponse(request.name, home)
    }

    override suspend fun search(query: String): List<SearchResponse> {
        mainUrl = FirebaseDomainHelper.getDomain("anisuge") ?: mainUrl
        Log.d("RaghavAnimeKitsu", "[AniSuge] search '$query' on $mainUrl")
        val encodedQuery = URLEncoder.encode(query, "UTF-8")
        val html = quickGet("$mainUrl/filter?keyword=$encodedQuery")
        Log.d("RaghavAnimeKitsu", "[AniSuge] search response html length ${html.length}")
        val soup = Jsoup.parse(html)
        ...
        Log.d("RaghavAnimeKitsu", "[AniSuge] search '$query' -> ${results.size} results")
        return results
    }

    override suspend fun load(url: String): LoadResponse? {
        mainUrl = FirebaseDomainHelper.getDomain("anisuge") ?: mainUrl
        Log.d("RaghavAnimeKitsu", "[AniSuge] load '$url'")
        val html = quickGet(url)
        Log.d("RaghavAnimeKitsu", "[AniSuge] load page html length ${html.length}")
        val soup = Jsoup.parse(html)

        val dataId = soup.selectFirst(".watch-wrap")?.attr("data-id")
            ?: Regex("""mangaId\s*=\s*(\d+)""").find(html)?.groupValues?.get(1)
            ?: return null
        Log.d("RaghavAnimeKitsu", "[AniSuge] load dataId=$dataId")
        ...
        Log.d("RaghavAnimeKitsu", "[AniSuge] load fetching episode list for dataId $dataId (title '$title')")
        ...
        Log.d("RaghavAnimeKitsu", "[AniSuge] episode list ajax response length ${epsResponseText.length}")
        ...
        Log.d("RaghavAnimeKitsu", "[AniSuge] episodes: ${subEpisodes.size} sub, ${dubEpisodes.size} dub for '$title'")
    ...
    override suspend fun loadLinks(
        ...
        Log.d("RaghavAnimeKitsu", "[AniSuge] loadLinks data '${data.take(100)}'")
        ...
        Log.d("RaghavAnimeKitsu", "[AniSuge] loadLinks ep $epNum ($selectedType) animeId $animeId on $baseUrl")
        ...
        Log.d("RaghavAnimeKitsu", "[AniSuge] server list ajax response length ${serverListResponseText.length}")
        ...
        for (st in serverTypes) {
            ...
            Log.d("RaghavAnimeKitsu", "[AniSuge] server-type '$typeAttr' match=$isMatch (want '$selectedType')")
        ...
        Log.d("RaghavAnimeKitsu", "[AniSuge] servers matched: ${serversToLoad.size} for '$selectedType' [${serversToLoad.joinToString { it.first }}]")
        if (serversToLoad.isEmpty()) return@coroutineScope false

        val loadedResults = serversToLoad.map { (serverName, linkId) ->
            async {
                Log.d("RaghavAnimeKitsu", "[AniSuge] loading server '$serverName' (linkId $linkId)")
                var loadedSingle = false
                val wrappedCallback: (ExtractorLink) -> Unit = { link ->
                    loadedSingle = true
                    Log.d("RaghavAnimeKitsu", "[AniSuge] server '$serverName' emitting link '${link.name}' -> ${link.url.take(120)}")
                    callback(link)
                }
                ...
                try {
                    ...
                    Log.d("RaghavAnimeKitsu", "[AniSuge] server '$serverName' player url: ${playerUrl.take(80)}")
                    ...
                    if (playerUrl.contains("plyr.php#")) {
                        ...
                        val decodedUrl = try {
                            ...
                            Log.e("RaghavAnimeKitsu", "[AniSuge] server '$serverName' plyr.php base64 decode failed: ${e.message}")
                            ""
                        }
                        if (decodedUrl.isNotBlank()) {
                            ...
                            Log.d("RaghavAnimeKitsu", "[AniSuge] server '$serverName' plyr.php decoded direct url: ${finalUrl.take(120)}")
                            wrappedCallback(
                                newExtractorLink(
                                    serverName,
                                    serverName,
                    ...
                    Log.d("RaghavAnimeKitsu", "[AniSuge] server '$serverName' isMegaplayClone=$isMegaplayClone embedBase=$embedBase")
                    ...
                    if (isMegaplayClone) {
                        try {
                            ...
                            Log.d("RaghavAnimeKitsu", "[AniSuge] server '$serverName' megaplay playerId=$playerId")
                            ...
                            if (playerId != null) {
                                ...
                                Log.d("RaghavAnimeKitsu", "[AniSuge] server '$serverName' getSources file: ${m3u8Url?.take(120)}")
                                ...
                                sourcesJson.tracks?.forEach { track ->
                                    ...
                                    if (track.kind == "captions" || track.kind == "subtitles") {
                                        Log.d("RaghavAnimeKitsu", "[AniSuge] server '$serverName' subtitle '${track.label ?: "Subtitle"}' -> ${file.take(120)}")
                                        subtitleCallback(
                            ...
                            Log.e("RaghavAnimeKitsu", "[AniSuge] server '$serverName' megaplay getSources failed: ${e.message}")
                        }
                        ...
                        if (!loadedSingle) {
                            try {
                                ...
                                Log.d("RaghavAnimeKitsu", "[AniSuge] server '$serverName' WebViewResolver fallback resolved: ${m3u8.take(120)}")
                                ...
                                Log.e("RaghavAnimeKitsu", "[AniSuge] server '$serverName' WebViewResolver fallback failed: ${e.message}")
                            }
                        }
                    } else {

                        val loaded = loadExtractor(playerUrl, "$baseUrl/", subtitleCallback, wrappedCallback)
                        Log.d("RaghavAnimeKitsu", "[AniSuge] server '$serverName' loadExtractor loaded=$loaded")
                        if (!loaded) {
                            try {
                                val resolver = com.lagradost.cloudstream3.network.WebViewResolver(
                                ...
                                Log.d("RaghavAnimeKitsu", "[AniSuge] server '$serverName' WebViewResolver resolved: ${resolved.take(120)}")
                                val headers = mapOf("Referer" to playerUrl)
                                when {
                                    resolved.contains(".m3u8", ignoreCase = true) -> {
                                        Log.d("RaghavAnimeKitsu", "[AniSuge] server '$serverName' resolved m3u8, generating links")
                                        val userAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36"
                                        M3u8Helper.generateM3u8(
                                            source = "$name - $serverName",
                                            streamUrl = resolved,
                                    ...
                                    resolved.contains(".mp4", ignoreCase = true) -> {
                                        Log.d("RaghavAnimeKitsu", "[AniSuge] server '$serverName' resolved mp4, emitting video link")
                                        wrappedCallback(
                                            newExtractorLink(
                                                source = "$name - $serverName",
                    ...
                    Log.e("RaghavAnimeKitsu", "[AniSuge] server '$serverName' failed: ${e.message}")
                }
                Log.d("RaghavAnimeKitsu", "[AniSuge] server '$serverName' done, loadedSingle=$loadedSingle")
                loadedSingle
            }
        }.awaitAll()
        Log.d("RaghavAnimeKitsu", "[AniSuge] loadLinks done: ${loadedResults.count { it }}/${loadedResults.size} servers yielded links")
