...
class AniWavesWebView(private val sourceName: String, private val baseUrl: String) : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        Log.d("RaghavAnimeKitsu", "[AniWaves][AniWavesWebView] getUrl source=$sourceName url=${url.take(120)} referer=$referer")
        runCatching {
            ...
            Log.d("RaghavAnimeKitsu", "[AniWaves][AniWavesWebView] WebView resolution attempt for ${url.take(120)}")
            val resolved = app.get(url, referer = referer ?: mainUrl, interceptor = resolver).url
            Log.d("RaghavAnimeKitsu", "[AniWaves][AniWavesWebView] resolved url=${resolved.take(120)}")
            val headers = mapOf("Referer" to url)
            when {
                resolved.contains(".m3u8", ignoreCase = true) -> {
                    Log.d("RaghavAnimeKitsu", "[AniWaves][AniWavesWebView] emitting m3u8 links: ${resolved.take(120)}")
                    generateM3u8(name, resolved, mainUrl, headers = headers).forEach(callback)
                }
                resolved.contains(".mp4", ignoreCase = true) -> {
                    Log.d("RaghavAnimeKitsu", "[AniWaves][AniWavesWebView] emitting mp4 link: ${resolved.take(120)}")
                    callback(
                        newExtractorLink(
                            source = name,
                            name = name,
...
class AniWavesEchoVideo : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        Log.d("RaghavAnimeKitsu", "[AniWaves][EchoVideo] getUrl url=${url.take(120)} referer=$referer")
        ...
        Log.d("RaghavAnimeKitsu", "[AniWaves][EchoVideo] html length=${html.length}")
        ...
        for (match in m3u8Matches) {
            ...
            Log.d("RaghavAnimeKitsu", "[AniWaves][EchoVideo] m3u8 link: ${m3u8Url.take(120)}")
            callback.invoke(
                newExtractorLink(
                    source = name,
                    name = name,
        ...
        Log.d("RaghavAnimeKitsu", "[AniWaves][EchoVideo] m3u8 matches=${m3u8Regex.findAll(html).count()}")
        ...
        for (match in mp4Matches) {
            ...
            Log.d("RaghavAnimeKitsu", "[AniWaves][EchoVideo] mp4 link: ${mp4Url.take(120)}")
            callback.invoke(
                newExtractorLink(
                    source = name,
                    name = name,
        ...
        Log.d("RaghavAnimeKitsu", "[AniWaves][EchoVideo] mp4 matches=${mp4Regex.findAll(html).count()}")
        val jsonSourceRegex = Regex(""""sources"\s*:\s*\[([^\]]+)\]""")
        val jsonMatch = jsonSourceRegex.find(html)
        Log.d("RaghavAnimeKitsu", "[AniWaves][EchoVideo] json sources block found=${jsonMatch != null}")
        if (jsonMatch != null) {
            ...
            for (urlMatch in urlRegex.findAll(sourcesJson)) {
                ...
                Log.d("RaghavAnimeKitsu", "[AniWaves][EchoVideo] json link: ${sourceUrl.take(120)}")
...
class AniWavesFilemoon : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        Log.d("RaghavAnimeKitsu", "[AniWaves][Filemoon] getUrl url=${url.take(120)} referer=$referer")
        val response = app.get(url, referer = referer ?: "https://aniwaves.ru/")
        val html = response.text
        Log.d("RaghavAnimeKitsu", "[AniWaves][Filemoon] html length=${html.length}")

        val packedRegex = Regex("""eval\(function\(p,a,c,k,e,d\).*?\)\)""", RegexOption.DOT_MATCHES_ALL)
        val packed = packedRegex.find(html)?.value
        Log.d("RaghavAnimeKitsu", "[AniWaves][Filemoon] packed script found=${packed != null} len=${packed?.length}")
        ...
        if (packed != null) {
            ...
            for (match in unpackedUrls) {
                Log.d("RaghavAnimeKitsu", "[AniWaves][Filemoon] packed m3u8 link: ${match.value.take(120)}")
                callback.invoke(
                    newExtractorLink(
                        source = name,
                        name = name,
        ...
        for (match in directM3u8) {
            Log.d("RaghavAnimeKitsu", "[AniWaves][Filemoon] direct m3u8 link: ${match.groupValues[1].take(120)}")
            callback.invoke(
                newExtractorLink(
                    source = name,
                    name = name,
...
class AniWavesMyVidPlay : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        Log.d("RaghavAnimeKitsu", "[AniWaves][MyVidPlay] getUrl url=${url.take(120)} referer=$referer")
        val response = app.get(url, referer = referer ?: "https://aniwaves.ru/")
        val html = response.text
        Log.d("RaghavAnimeKitsu", "[AniWaves][MyVidPlay] html length=${html.length}")

        val m3u8Regex = Regex("""(?:file|src|source)\s*[:=]\s*["'](https?://[^"']*\.m3u8[^"']*)["']""")
        for (match in m3u8Regex.findAll(html)) {
            Log.d("RaghavAnimeKitsu", "[AniWaves][MyVidPlay] m3u8 link: ${match.groupValues[1].take(120)}")
            callback.invoke(
                newExtractorLink(
                    source = name,
                    name = name,
        ...
        val jsonSourceRegex = Regex(""""sources"\s*:\s*\[([^\]]+)\]""")
        val jsonMatch = jsonSourceRegex.find(html)
        Log.d("RaghavAnimeKitsu", "[AniWaves][MyVidPlay] json sources block found=${jsonMatch != null}")
        if (jsonMatch != null) {
            val urlRegex = Regex(""""(?:file|url)"\s*:\s*"([^"]+)"""")
            for (urlMatch in urlRegex.findAll(jsonMatch.groupValues[1])) {
                val sourceUrl = urlMatch.groupValues[1].replace("\\/", "/")
                Log.d("RaghavAnimeKitsu", "[AniWaves][MyVidPlay] json link: ${sourceUrl.take(120)}")
                val type = if (sourceUrl.contains(".m3u8")) ExtractorLinkType.M3U8 else ExtractorLinkType.VIDEO
                callback.invoke(
                    newExtractorLink(
                        source = name,
